...
## 工夫した点
① Activityごとの役割分担
AddActivity・EditActivityなど、機能ごとに画面を分けることで
コードの可読性と保守性を意識しました。
② SQLiteを用いたデータ管理
INSERT
UPDATE
DELETE
SELECT（検索含む）
CRUD処理をすべて自分で実装し、データベース操作の理解を深めました。
③ 検索機能の実装
キーワード/カテゴリー/調理時間/お気に入り
キーワードではレシピ名で部分一致検索ができるようにしました。
ユーザーが目的のレシピを素早く見つけられる設計にしています。
④ 写真登録機能の追加（発展機能）
...
文字情報だけでなく、画像も扱えるアプリへ拡張しました。
データ型の違いを意識した設計を行いました。
⑤ ユーザビリティの向上
未入力チェックの実装
Toastによるエラーメッセージ表示
編集画面で既存データを自動表示
